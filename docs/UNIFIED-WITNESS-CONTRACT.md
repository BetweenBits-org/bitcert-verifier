...
54..86  32   aux_commitment          ( zero for MVP: 00 × 32 )
------  ---
total   86
```
...
`merkle_root = b[22:54]`, `aux = b[54:86]`; length **MUST** be exactly 86.
...
>
> **Reference: `BC01` legacy placeholder (53 B)** — still accepted by the
> verifier for older anchors: `magic "BC01"(4) | version 0x01(1) | merkle_root(32,
...
When `decode_op_return` succeeds (BC01 or BC30) **and** `witness_envelope` is
present:
...
3. `op_return_payload_hex` is a **BC30** payload (decodes via §2), **not** a raw
...
op_return_payload_hex (BC30, 86 B) =
  42 43 33 30          "BC30"
  1e                   version 30
