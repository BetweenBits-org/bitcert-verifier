...
The verifier recovers the PDF straight from the witness and binds it through the chain `body → leaf_bytes → merkle_root → OP_RETURN → txid`:
...
Because the witness is **malleable** (not committed to the txid), a tampered witness body leaves the txid and OP_RETURN intact (steps 1–2 still pass) but fails step 3 (`sha256(body) ≠ leaf_bytes`) — see `../08-tampered-witness/` and `fixtures/07-witness-unified.tampered.json`.
