...
Same unified bundle as example 07, but one bit of the **witness body** (the inscribed PDF) was flipped. The witness is NOT committed to the txid, so the `reveal_txid` and the `payload(merkle_root)` anchor output are unchanged - steps 1 (txid binding) and 2 (record binds to the on-chain merkle_root) still pass. But the recovered body no longer hashes to `record.leaf_bytes`, so step 3 FAILS.
